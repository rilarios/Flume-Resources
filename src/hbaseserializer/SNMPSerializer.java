 ...
 * Este es un ejemplo que utiliza información de SNMP pero dependiendo de los datos el serializer tendrá que cambiar.<br><br>
 * 
 ...
 *
 */
public class SNMPSerializer implements HbaseEventSerializer  {
	...
	public List<Row> getActions() {
		...
		if(payloadColumn != null){
			...
			try {
				...
				put.add(columnFamily, payloadColumn, buildPayload(payload));
				actions.add(put);
			} catch (Exception e){
	...
	private byte[] buildPayload(byte[] Apayload) {
		String values = "";
		String pl = new String(Apayload);
		String[] campos = pl.split(",");
		for(int i = 3; i< campos.length ; i++){
			values = values + campos[i] + ",";
		}
		return values.substring(0, values.length()-1).getBytes();
	}
	...
	private byte[] getrowKeyFromPayload(byte[] payLoad) {
		...
		String[] campos = pl.split(",");
		//hostname(2) | date (1) | timestamp | UUID(autogen) 
		return (campos[2] + "|" + campos[1] + "|" + System.currentTimeMillis() + "|" + uuid.toString()).getBytes();
